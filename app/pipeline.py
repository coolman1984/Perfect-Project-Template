"""UNIVERSAL CORE — legacy single-source compatibility pipeline.

The V8.1 project model is multi-source. This class remains the proven executor
for each legacy/reference source while project-level orchestration is added.
It must not contain department vocabulary or KPI formulas.
"""
...
from app.quality.engine import BLOCK, QualityEngine, QualityReport
...
class ReportContract:
    def __init__(self, directory: Path) -> None:
        self.directory=Path(directory); self.report=tomllib.loads((self.directory/"report.toml").read_text("utf-8")); self.pipeline=tomllib.loads((self.directory/"pipeline.toml").read_text("utf-8")); name=str(self.pipeline.get("dashboard",{}).get("config","dashboard.toml")); self.dashboard=load_dashboard_config(self.directory/name)
    @property
...
@dataclass
class RunOutcome:
    run_id:str; status:str; quality_status:str; history:HistoryResult=field(default_factory=HistoryResult); rows_extracted:int=0; rows_rejected:int=0; rows_filtered:int=0; control_totals:list[ControlTotal]=field(default_factory=list); quality_report:QualityReport|None=None; dashboard:dict[str,Any]|None=None; error_code:str|None=None; error_message:str|None=None; demo_data:bool=False; quarantine:dict[str,int]=field(default_factory=dict)
    @property
...
class Pipeline:
    def __init__(self,database:Database,contract:ReportContract,*,application_version:str="0.0.0")->None:
        self.database=database; self.contract=contract; self.application_version=application_version; self.sql=SqlRunner(database,contract.sql_directory); self.analytics=ConfiguredAnalytics(self.sql,contract.dashboard,metrics_file=contract.sql_file("metrics"),insights_file=contract.sql_file("insights"))
    ...
    def run(self,run_id:str,port:Any,*,requested_period:str|None=None)->RunOutcome:
        ...
        try:
            ...
            outcome.rows_extracted=self._stage(run_id,port); machine.to("STAGING"); machine.to("VALIDATING")
            ...
            machine.to("CLEANING"); machine.to("UPDATING_HISTORY"); outcome.history=self._update_history(run_id,requested_period); machine.to("ARCHIVING"); self._archive(); machine.to("CALCULATING"); package=self.analytics.run(); machine.to("BUILDING_INSIGHTS"); machine.to("BUILDING_JSON"); outcome.dashboard=json_builder.build(contract=self.contract,analytics=package,outcome=outcome)
    ...
    def _stage(self,run_id:str,port:Any)->int:
        return StagingWriter(self.database,self.contract.table("raw"),self.contract.business_columns).stage(port,port.lineage()).rows_staged
    def _validate(self,run_id:str,outcome:RunOutcome)->tuple[QualityReport,Population,list[ControlTotal]]:
        quality=QualityReport(run_id=run_id); engine=QualityEngine(self.database,self.contract.report); raw=self.contract.table("raw"); checks=self.contract.sql_file("checks"); clean_sql=self.contract.sql_file("clean"); present=[c for c in self.database.columns_of(raw) if c not in LINEAGE_COLUMNS]; staged=[c for c in present if (self.database.scalar(f"SELECT count(*) FROM {raw} WHERE _run_id = ? AND {c} IS NOT NULL",[run_id]) or 0)>0]; control_column=self.contract.report["quality"]["control_total_column"]
        ...
        engine.check_no_blank_business_key(quality,raw,run_id,self.contract.key_columns); rejected=self._quarantine_rejects(run_id,clean_sql); filtered=self._quarantine_duplicates(run_id,clean_sql); outcome.rows_rejected=rejected; outcome.rows_filtered=filtered; self.sql.run_script(clean_sql,"build_clean",[run_id]); engine.check_duplicate_business_keys(quality,self.contract.table("clean"),run_id,self.contract.key_columns); non_negative=list(self.contract.report.get("quality",{}).get("non_negative_columns",[]))
        ...
        source=int(self.sql.scalar(checks,"source_row_count",[run_id]) or 0); accepted=int(self.sql.scalar(clean_sql,"clean_row_count",[run_id]) or 0); population=Population(source,accepted,rejected,filtered,(("exact_duplicate",filtered),)); check_population(quality,population); source_value=as_decimal(self.sql.scalar(checks,"source_control_total",[run_id])); accepted_value=as_decimal(self.sql.scalar(checks,"accepted_control_total",[run_id])); rejected_value=as_decimal(self.sql.scalar(checks,"rejected_control_total",[run_id])); filtered_value=as_decimal(self.sql.scalar(checks,"filtered_control_total",[run_id])); control=ControlTotal(control_column,source_value,accepted_value+rejected_value+filtered_value); check_control_total(quality,control); outcome.quarantine=summary_by_reason(self.database,run_id); return quality,population,[control]
    def _quarantine_rejects(self,run_id:str,clean_sql:str)->int:
        ...
        rows=self.sql.run_named(clean_sql,"negative_measure_rows",[run_id]); return quarantine_rows(self.database,run_id,self.contract.report_id,[QuarantinedRow(source_row=r[0],reason_code=NEGATIVE_MEASURE,raw_values=dict(zip(self.contract.business_columns,r[1:]))) for r in rows])
    def _quarantine_duplicates(self,run_id:str,clean_sql:str)->int:
        ...
        rows=self.sql.run_named(clean_sql,"exact_duplicate_rows",[run_id]); return quarantine_rows(self.database,run_id,self.contract.report_id,[QuarantinedRow(source_row=r[0],reason_code=EXACT_DUPLICATE,raw_values=dict(zip(self.contract.business_columns,r[1:]))) for r in rows])
    def _update_history(self,run_id:str,requested_period:str|None)->HistoryResult:
        history=self.contract.report["history"]
        return HistoryEngine(self.database,history_table=self.contract.table("history"),clean_table=self.contract.table("clean"),business_columns=self.contract.business_columns,key_columns=self.contract.key_columns,event_date_column=history["event_date"]).apply(run_id,load_mode=self.contract.report["load_mode"],lookback_days=int(history.get("lookback_days") or 0),deletion_rule=history.get("deletion_rule","ignore"),requested_period=requested_period)
    def _archive(self)->None: archive_history(self.database,history_table=self.contract.table("history"),report_id=self.contract.report_id,archive_root=Path("workspace")/"archive"/self.contract.report_id,event_date_column=self.contract.report["history"]["event_date"])
