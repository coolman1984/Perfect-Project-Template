...
def _kind(value: Any) -> str:
    ...
    if _INTEGER.match(text):
        return "integer_like_text"
    if _DECIMAL.match(text):
        return "number_like_text"
    return "text"


def profile_chunks(
    chunks: Iterable[Any], *, distinct_cap: int = 10_000,
    include_samples: bool = False, sample_limit: int = 3,
) -> SourceProfile:
    """Profile chunked data without loading business rows into AI context.

    Samples are excluded by default. The returned profile contains structure,
    counts and type hints, which is enough for an agent's first mapping pass in
    most projects while keeping confidential values out of the prompt.
    """
    columns: list[str] = []
    ...
    for chunk in chunks:
        ...
        if not columns:
            ...
            states = {
                name: {
                    "non_null": 0, "nulls": 0, "types": {}, "distinct": set(),
                    "capped": False, "samples": [],
                }
                for name in columns
            }
        elif chunk_columns != columns:
            raise ValueError(
                "schema changed between chunks; profile each schema separately "
                "instead of silently merging columns"
            )
    ...
    for name in columns:
        state = states[name]
        distinct = None if state["capped"] else len(state["distinct"])
        profiles.append(ColumnProfile(
            name=name,
            non_null=state["non_null"],
            nulls=state["nulls"],
            inferred_types=dict(sorted(state["types"].items())),
            distinct_count=distinct,
            distinct_capped=state["capped"],
            candidate_unique_key=(
                row_count > 0 and state["nulls"] == 0 and not state["capped"]
...
def compact_text(profile: SourceProfile) -> str:
    lines = [
        f"Rows: {profile.rows}", f"Columns: {profile.columns}", "",
        "| Column | Non-null | Null | Distinct | Types | Candidate unique key |",
        "|---|---:|---:|---:|---|---|",
    ]
    for column in profile.column_profiles:
        ...
        lines.append(
            f"| {column.name} | {column.non_null} | {column.nulls} | {distinct} | "
            f"{types} | {'yes' if column.candidate_unique_key else 'no'} |"
        )
    lines += [
        "",
        "Samples were explicitly enabled for this profile."
        if profile.sample_values_included
        else "Raw sample values are intentionally omitted.",
    ]
    return "\n".join(lines) + "\n"
